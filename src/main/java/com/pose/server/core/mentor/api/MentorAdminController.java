...
public class MentorAdminController {
    ...
    public String viewApplications(@SessionAttribute(name = "loginUser") MemberEntity loginUser,
                                   Model model) {
    ...
    public String approve(@PathVariable Long applyId,
                          @SessionAttribute(name = "loginUser") MemberEntity loginUser) {
        if (!loginUser.isAdmin()) return "error/403";
