...
public class MentorApplyController {
     ...
     *
     * @param user 세션에서 가져온 로그인 사용자 정보
     * @param dto       사용자 입력값 (경력, 소속 등)
     */
    @PostMapping
    public String apply(@SessionAttribute(name = "user", required = true) MemberEntity user,
                        @ModelAttribute MentorApplyDto dto,
                        RedirectAttributes redirectAttributes) {
        try {
            mentorApplyService.applyForMentor(user.getUserId(), dto);
